...
#ifelse statement to create new column called neighbor. If focal= corynactis and shorthand= mefr or stpu, then this value= 1. If not= 0
glm_data$neighbor <- ifelse(glm_data$focal=="corynactis", 
                      ifelse(glm_data$shorthand=="mefr" | glm_data$shorthand=="stpu",1,0),0)
#View(glm_data)
sort(unique(glm_data$neighbor))
length(which(glm_data$neighbor==1)) #212

#check if total number of coca neighbor with urchin is equal to total number urchin neighbor with coca. 
glm_data$neighbor2 <- ifelse(glm_data$focal=="strongylocentrotus"|glm_data$focal =="mesocentrotus", ifelse(glm_data$shorthand=="coca",1,0),0)
sort(unique(glm_data$neighbor2))
length(which(glm_data$neighbor2==1)) #207. Not equal to 212, but close. Difference due to errors when entering data 
...
#glm practice models  
glm1 <- glm(stand_enso ~ year, data=glm_data, family="binomial")
summary(glm1) 

glm2 <- glm(stand_enso ~ year, data=glm_data)
summary(glm2) #2015 and 2016 significant 
...
glm3 <- glmer(neighbor ~ (1|enso), data=glm_data, family="binomial")
summary(glm3) #significant 
```
```{r}
...
```{r}
#GLM mixed effects model for season 
library(lme4)
glm_data$stand_upwell <- as.numeric(glm_data$stand_upwell)
glm4 <- glmer(stand_upwell ~ (1|season), data=glm_data)
summary(glm4)
print(glm4)
ranef(glm4)

#glm for enso and upwell not standardized 
glm5 <- glmer(neighbor ~ (1|enso) + (1|upwell), data=glm_data, family="binomial" )
summary(glm5)
```

```{r}
#binomial glm for non standardized neighbor events by multiple factors (season, year, location)
glm_data$neighbor <- as.numeric(glm_data$neighbor)
glm6 <- glm(neighbor ~ year + season, data=glm_data, family="binomial")
summary(glm6)
glm7 <- glm(neighbor ~ year + season + location, data=glm_data, family="binomial")
summary(glm7) #lower AIC than glm6 or glm8, so better model
glm8 <- glmer(neighbor ~ (1|enso) + (1|upwell) + (1|location), data=glm_data, family="binomial" )
summary(glm8) # not useful 

glm_data$filename<- as.numeric(glm_data$filename)
glm9 <- glmer(neighbor ~ enso + upwell + location + (1|filename) , data=glm_data, family= "binomial")
summary(glm9) #upwelling happening and location naples significant predictors of neighbor events 

glm10 <- glm(neighbor ~ year + season , data=glm_data, family="binomial")


#test if data fit model assumptions 
#install.packages("DHARMa")
library(DHARMa)
simulationOutput <- simulateResiduals(fittedModel = glm9, plot = T) #gives message to use testOutliers and bootstrap, significant deviation

testOutliers(simulationOutput = glm9, margin = c("both"), type = c("bootstrap"), plot = T) #p value < 0.05 so no overdispersion/outliers
?testOutliers
...
library(see)
binned_residuals(glm9) #38% model not a great fit 
testDispersion(glm9)# >0.05, so there is no overdispersion  
binned_residuals(glm7) #88% of the residuals are inside the error bounds. model is a good fit 
binned_residuals(glm5) #50% 
...
```{r}
glm_all <- glm(neighbor ~ year + upwell + season + location + enso, data=glm_data, family="binomial")
stepAIC(glm_all, direction="backward") #for now, this is saying season and location is most predictive (but also, no random effects accounted for so take this with a grain of salt)
...
glm_11 <- glm(neighbor ~ upwell + location, data=glm_data, family="binomial")
summary(glm_11)
...
#next- how to report data, visualizing 
```
