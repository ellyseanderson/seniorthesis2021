...
```{r}
glm_all <- glm(neighbor ~ year + season + location + enso + upwell, data=glm_data, family="binomial")
stepAIC(glm_all, direction="both") #for now, this is saying season and location is most predictive (but also, no random effects accounted for so take this with a grain of salt)

glm_11 <- glm(neighbor ~ season + location, data=glm_data, family="binomial")
summary(glm_11)
